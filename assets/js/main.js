...
alertDelete.addEventListener("click", (event) => {
    if (window.confirm("すべて削除されますがよろしいですか？")) {
        window.location.href = "access-all-delete.php?date=" + alertDelete.dataset.date;
    } else {
        // なにもしない
    }
})
...
for (const value of itemDelete) {
    value.addEventListener("click", () => {
        if (window.confirm("1件削除してもよろしいですか？")) {
            window.location.href = "access-item-delete.php?getId=" + value.title + "&page=" + alertDelete.dataset.date;
        } else {
            // 何もしない
        }
    });
